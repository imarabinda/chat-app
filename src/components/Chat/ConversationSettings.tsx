...
const ConversationSettings: FC<ConversationConfigProps> = ({
  ...
  const handleFileInputChange = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const downloadURL = await groupProfileImageUpload(file, conversationId);
  ...
  const leaveGroup = async (noRedirect?: boolean, isClosed?: boolean) => {
    await updateDoc(
      doc(
        firestore,
        FIRESTORE_CONVERSATIONS_COLLECTION,
  ...
  const clearAndCreate = useCallback(async () => {
    if (currentUser?.isSiteAdmin) {
      ...
      await leaveGroup(true, true);
    }

    if (!conversation.isClosed && currentUser) {
  ...
  }, [currentUser, conversationId, conversation]);
