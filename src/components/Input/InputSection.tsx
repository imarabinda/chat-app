...
import { database, firestore, storage } from "../../shared/firebase";
import {
  getDownloadURL,
  ref as storageRef,
  uploadBytes,
...
import { getConversationReference, getUtcTime } from "../../shared/helpers";
...
const InputSection: FC<InputSectionProps> = ({
  ...
  const referencePath = useMemo(() => {
    return `${getConversationReference(conversationId as string)}/isTyping/${
      currentUser?.uid
    }`;
  ...
  useEffect(() => {
    ...
    if (isTyping) {
      let reference = ref(database, referencePath);
      if (isTyping == "typing") {
        databaseSet(reference, true);
      }
      if (isTyping == "stopped") {
        remove(reference);
      }
  ...
  const uploadFile = async (file: File) => {
    try {
      ...
      const fileReference = storageRef(storage, formatFileName(file.name));
  ...
  useEffect(() => {
    ...
    const dropFileHandler = async (e: any) => {
      ...
      let selectedFiles = [...previewFiles];
  ...
  return (
    <>
      ...
      {isClosed && (
        <div
          className={`border-dark-lighten pointer-events-none flex h-16 select-none items-stretch gap-1 border-t px-4`}
        >
          <span>
            Conversation is closed, you can delete it,{" "}
            <a
              onClick={(e) => {
                e.preventDefault();
                deleteDoc(
